...
def make_graphs(threshold):
    ...
    for path in l:
        m = np.loadtxt('FourBoxRawData/' + path, delimiter=',')
        n_tot = m.shape[0] - 1
        ...
        nx.write_gml(my_graph, 'Graphs/{}.gml'.format(filename[:-4]))
        ...
        weight_dict = dict(zip(index_named, weights))
        nx.set_edge_attributes(my_graph, weight_dict, "value")
        nx.write_gml(my_graph, 'Graphs/{}_same.gml'.format(filename[:-4]))
        ...
        weight_dict = dict(zip(index_named, weights))
        nx.set_edge_attributes(my_graph, weight_dict, "value")
        nx.write_gml(my_graph, 'Graphs/{}_cross.gml'.format(filename[:-4]))
