...
def correlation_score_matrix_above_threshold(path_ctx, threshold=0):
    ...
    partaking_neurons_per_event_scope1 = np.loadtxt('/home/romano/mep/ContinuousGlobalSynchrony/SynchronousEventParticipatingNeurons/{}'.format(path_ctx).replace("ctx", "cbl"), delimiter=',')    
    partaking_neurons_per_event_scope2 = np.loadtxt('/home/romano/mep/ContinuousGlobalSynchrony/SynchronousEventParticipatingNeurons/{}'.format(path_ctx), delimiter=',')    
...
def export_four_box_plots():
    ...
    for file_name_cerebellum, file_name_cortex in zip(cerebellum_files, cortex_files):
        ...
        correlation_score = correlation_score / np.amax(correlation_score)
        ...
        axes[0, 0].imshow(m, aspect='auto', vmin=0, vmax=1)
        ...
        m = correlation_score[:scope1_valid, scope1_valid:]
        axes[0, 1].imshow(m, aspect='auto', vmin=0, vmax=1)
        axes[0, 1].xaxis.tick_top()
        axes[0, 1].set_xlabel('Cortex')
        axes[0, 1].xaxis.set_label_position('top')
        
        m = correlation_score[scope1_valid:, :scope1_valid]
        axes[1, 0].imshow(m, aspect='auto', vmin=0, vmax=1)
        axes[1, 0].set_ylabel('Cortex')

        m = correlation_score[scope1_valid:, scope1_valid:]
        axes[1, 1].imshow(m, aspect='auto', vmin=0, vmax=1)
