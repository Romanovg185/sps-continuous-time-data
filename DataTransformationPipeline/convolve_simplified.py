...
:returns: A list of tuples of the [(peak_area[0], peak_amplitudes[0], peak_onset_time[0], peak_end_time[0]), ...]
"""
def convolve_with_kernel_ground_truth(m_cbl, m_ctx, n_samples=1):
    # Make a continuous (dt=0.001) sum of kernel-convolved spike onset times
    ...
    z = np.sum(y, axis=0)
    threshold = get_ground_truth_threshold_criterion(m, n_samples)
...
"""
def get_indices_arbitrary_overlap(m_cbl, m_ctx, is_using_ground_truth=False):
    n_samples = 1 # Number of samples of ground truth
    if is_using_ground_truth:
        areas, amplitudes, starts, ends = convolve_with_kernel_ground_truth(m_cbl, m_ctx, n_samples)
    else:
...
"""
def locate_indices_neuron_per_pattern(m_cbl, m_ctx):
    ind = get_indices_arbitrary_overlap(m_cbl, m_ctx)
...
def find_participants_both(is_thresholding=False):
    ...
    for file_name_cerebellum, file_name_cortex in zip(cerebellum_files, cortex_files):
        ...
        i, participator_cbl, participator_ctx = locate_indices_neuron_per_pattern(m_cbl, m_ctx) # [cell, significant_event]

        # Splitting cortex and cerebellum
        print(file_name_cortex)
        print(file_name_cortex[11:])
        np.savetxt('/home/romano/mep/ContinuousGlobalSynchrony/SynchronousEventParticipatingNeurons/cbl' + file_name_cortex[11:], participator_cbl.T, delimiter=',')
        np.savetxt('/home/romano/mep/ContinuousGlobalSynchrony/SynchronousEventParticipatingNeurons/ctx' + file_name_cortex[11:], participator_ctx.T, delimiter=',')
        np.savetxt('/home/romano/mep/ContinuousGlobalSynchrony/IntervalsSignificantCorrelation/' + file_name_cortex[12:], np.array(i), delimiter=',')
