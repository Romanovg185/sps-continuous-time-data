...
from DataTransformationPipeline.ground_truth import get_ground_truth_threshold_criterion
import os

### Free parameters ###
kernel_steepness = 0.100

def stack_cerebellum_cortex(m_cbl, m_ctx):
...
"""
def convolve_with_kernel_two_sigma(m_cbl, m_ctx):
    # Make a continuous (dt=0.001) sum of kernel-convolved spike onset times
    m = stack_cerebellum_cortex(m_cbl, m_ctx)
...
"""
Convolve with an Epanechikov kernel
Uses the ground truth as defined in ground_truth.py
:param m: Raw input spikes in a square matrix, padded with np.nans
:param n_sds: Number of standard deviations for threshold
:returns: A list of tuples of the [(peak_area[0], peak_amplitudes[0], peak_onset_time[0], peak_end_time[0]), ...]
"""
def convolve_with_kernel_ground_truth(m_cbl, m_ctx, n_sds, n_samples=1):
    # Make a continuous (dt=0.001) sum of kernel-convolved spike onset times
    m = stack_cerebellum_cortex(m_cbl, m_ctx)
    u = np.arange(-1*kernel_steepness, kernel_steepness + 0.001, 0.001)
    kernel = 4*kernel_steepness/3*(1 - (u/kernel_steepness)**2)
    ...
    for i in range(m.shape[1]): # with i as cell index
        ...
        y[i, indices_of_spike_nonan] = 1
        y[i, :] = np.convolve(y[i, :], kernel, mode='same')
        print('r{}/{}'.format(i+1, m.shape[1]))
    z = np.sum(y, axis=0)
    threshold = get_ground_truth_threshold_criterion(m_cbl, m_ctx, n_sds, n_samples)

    # Indentify peaks as regions of z such that all values are Nsigma significantly above the mean, ending when a value lower than the mean is reached
    peaks = [[]]
    indices_peak_starts = []
    indices_peak_ends = []
    for index, i in enumerate(z):
...
"""
def get_indices_arbitrary_overlap(m_cbl, m_ctx, n_sds, is_using_ground_truth=True):
    n_samples = 1 # Number of samples of ground truth
    if is_using_ground_truth:
        areas, amplitudes, starts, ends = convolve_with_kernel_ground_truth(m_cbl, m_ctx, n_sds, n_samples)
    else:
        areas, amplitudes, starts, ends = convolve_with_kernel_two_sigma(m_cbl, m_ctx)
...
"""
def locate_indices_neuron_per_pattern(m_cbl, m_ctx, n_sds):
    ind = get_indices_arbitrary_overlap(m_cbl, m_ctx, n_sds)
    print("Indices generated")

    patterns_cbl = []
    for start, end in ind:
        single_pattern = []
        for i, cell in enumerate(m_cbl.T):
            right_of_start = cell > start
            left_of_end = cell < end
            if np.any(np.logical_and(left_of_end, right_of_start)):
                single_pattern.append(i)
        patterns_cbl.append(tuple(single_pattern))

    patterns_ctx = []
    for start, end in ind:
        single_pattern = []
        for i, cell in enumerate(m_ctx.T):
            right_of_start = cell > start
            left_of_end = cell < end
            if np.any(np.logical_and(left_of_end, right_of_start)):
                single_pattern.append(i)
        patterns_ctx.append(tuple(single_pattern))

    participator_cbl = np.zeros((len(ind), m_cbl.shape[1]))
    for i, cells in enumerate(patterns_cbl):
        participator_cbl[i, cells] = 1
    participator_ctx = np.zeros((len(ind), m_ctx.shape[1]))
    for i, cells in enumerate(patterns_ctx):
        participator_ctx[i, cells] = 1
    return ind, participator_cbl, participator_ctx
...
"""
def find_participants_both(n_sds):
    ...
    for file_name_cerebellum, file_name_cortex in zip(cerebellum_files, cortex_files):
        ...
        i, participator_cbl, participator_ctx = locate_indices_neuron_per_pattern(m_cbl, m_ctx, n_sds) # [cell, significant_event]

        # Splitting cortex and cerebellum
        print(file_name_cortex)
        print(file_name_cortex[11:])
        np.savetxt('SynchronousEventParticipatingNeurons/cbl' + file_name_cortex[11:], participator_cbl.T, delimiter=',')
        np.savetxt('SynchronousEventParticipatingNeurons/ctx' + file_name_cortex[11:], participator_ctx.T, delimiter=',')
        np.savetxt('IntervalsSignificantCorrelation/' + file_name_cortex[12:], np.array(i), delimiter=',')
